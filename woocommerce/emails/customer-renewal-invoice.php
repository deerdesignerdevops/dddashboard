...
<?php elseif ( $order->has_status( 'failed' ) ) : ?>
	...
	sprintf(
		// translators: %1$s: name of the blog, %2$s: link to checkout payment url, note: no full stop due to url at the end
		_x( 'The automatic payment to renew your subscription with %1$s has failed. Your design team is currently on standby for payment confirmation before delivering any designs in the pipeline. If you need to update your credit card details, click on Billing Portal on the dashboard. To avoid any interruption on the service, please log in and pay for the renewal from your account page: %2$s', 'In customer renewal invoice email', 'woocommerce-subscriptions' ),
		esc_html( get_bloginfo( 'name' ) ),
		'<a href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . esc_html__( 'Pay here &raquo;', 'woocommerce-subscriptions' ) . '</a>'
	), array( 'a' => array( 'href' => true ) ) ); ?>
	</p>
<?php endif; ?>
