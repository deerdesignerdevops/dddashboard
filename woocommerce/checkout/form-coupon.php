...
<form class="checkout_coupon woocommerce-form-coupon" method="post" style="display:none">
	...
	<?php
		// Exibe os cupons aplicados e uma opção para removê-los
		if ( count( WC()->cart->get_applied_coupons() ) > 0 ) {
			$couponsApplied = WC()->cart->get_applied_coupons();

			foreach( $couponsApplied as $coupon ) { ?>
				<p class="dd__remove_btn">
					<a href="<?php echo esc_url( add_query_arg( 'remove_coupon', $coupon, wc_get_cart_url() ) ); ?>" class="button">
						<?php printf( esc_html__( 'Remove Coupon: %s', 'woocommerce' ), esc_html( $coupon ) ); ?>
					</a>
				</p>
			<?php }
		}
